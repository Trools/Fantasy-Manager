    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div className="bg-[--color-bg-surface] rounded-lg border border-[--color-border-default] overflow-hidden">
            ...
            {sortedParticipants.length > 0 ? (
              sortedParticipants.map((p) => (
                ...
                >
                  ...
                  {/* Name */}
                  <span className="text-[--color-text-primary] flex-1">
                    {p.username}
                  </span>
                  ...
                  <div className="flex gap-2">
                    {p.user_id === user?.id && (
                      <span className="text-xs px-2 py-0.5 rounded bg-[--color-accent-primary]/20 text-[--color-accent-primary]">
                        You
                      </span>
                    )}
                  </div>
              ...
              <div className="px-4 py-8 text-center text-[--color-text-muted]">
          ...
          {!hasOrder && (
            <div className="px-4 py-3 bg-[--color-bg-elevated] text-sm text-[--color-text-secondary]">
        ...
        <div className="bg-[--color-bg-surface] rounded-lg border border-[--color-border-default] overflow-hidden">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-[--color-text-secondary]">Picks per person</span>
                <div className="text-[--color-text-primary] font-medium">
                  {settings.total_picks}
                </div>
              </div>
              <div>
                <span className="text-[--color-text-secondary]">Time per pick</span>
                <div className="text-[--color-text-primary] font-medium">
                  {settings.seconds_per_pick}s
                </div>
              </div>
              <div>
                <span className="text-[--color-text-secondary]">Order mode</span>
                <div className="text-[--color-text-primary] font-medium capitalize">
                  {settings.order_mode}
                </div>
              </div>
              <div>
                <span className="text-[--color-text-secondary]">Max per country</span>
                <div className="text-[--color-text-primary] font-medium">
            ...
            <div>
              <span className="text-sm text-[--color-text-secondary] block mb-2">
              ...
              <div className="grid grid-cols-4 gap-2 text-sm">
                {POSITIONS.map((pos) => (
                  ...
                  >
                    <div className="text-[--color-text-muted] text-xs">{pos}</div>
                    <div className="text-[--color-text-primary] font-medium">
          ...
          {!isAdmin && (
            <div className="px-4 py-3 bg-[--color-bg-elevated] text-sm text-[--color-text-muted] text-center">
